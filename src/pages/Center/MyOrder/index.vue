    ...
    <div class="order-content">
      ...
      <div class="orders">
        <table class="order-item" v-for="(item) in orderList.records" :key="item.id">
          ...
          <tbody>
            <tr v-for="(item1,index) in item.orderDetailList" :key="item1.index">
              <td width="60%" >
                <div class="typographic">
                  <img :src="item1.imgUrl" style="width:50px;height:50px"/>
              ...
              <td :rowspan="item.orderDetailList.length" width="8%" class="center" v-if="index==0">
                <a href="#" class="btn" v-if="index==0">已完成 </a>
              </td>
              <td :rowspan="item.orderDetailList.length" width="13%" class="center" v-if="index==0">
                <ul class="unstyled">
                  <li>
...
export default {
  ...
  methods: {
    async getData() {
      let { page, limit } = this;
      const result =await this.$api.reqOrderList(page, limit); // 在向服务器要请求的时候一定要加await 来等待结果的返回
      console.log("reqOrderList--------", result);
      if (result.code == 200) {
