...
export default {
  ...
  methods: {
    async getPayInfo() {
      let result = await this.$api.reqPayInfo(this.$route.params.id);
      if (result.code == 200) {
    ...
    async open() {
     ...
     if(!this.timer){
        this.timer=setInterval(async()=>{
          // 发送请求
          let payStatus=await this.$api.reqorderPayInfo(this.$route.params.id)
