  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="item in addressInfo" :key="item.id">
        ...
        <!-- <p @click="changeDefalut(addressInfo,item)> -->
        <p @click="changeDefalut(addressInfo, item)">
      ...
      <div class="detail">
        ...
        >
          <li>
            <img :src="item.imgUrl" style="width: 200px; height: 150px" />
          </li>
          <li>
            <p>
              {{ item.skuName }}
...
export default {
  name: "Trade",
  mounted() {
    this.$store.dispatch("useAddressInfo");
  ...
  computed: {
    ...
    userDefaultAddress() {   
      // this.addressInfo.forEach((element) =>element.isDefault == 1
      // );
      console.log("this.addressInf",this.addressInfo.find(item=>item.isDefault==1))
  ...
  methods: {
    totalPrice() {
      let sum = 0;
      this.orderInfo.detailArrayList.forEach((element) => {
        sum += element.orderPrice * element.skuNum;
      });
      return sum;
    },
    changeDefalut(addressInfo, item) {
   ...
   async submitOrder() {
      ...
      let data = {
        ...
        orderComment: this.$refs.cur.value, //买家的留言信息
        orderDetailList: this.orderInfo.detailArrayList, //商品清单
      };

      console.log("trederNO----",tradeNo)
      console.log("data----",data)
      //准备提交的接口
         let result = await this.$api.reqSummitOrder(tradeNo,data)
         console.log("reqSummitOrder---",result)
         if(result.code==200){
